            ...
            getVideoSrc: function(vid) {
                ...
                $http.jsonp("http://play.youku.com/play/get.json?vid=" + vid + "&ct=12&callback=JSON_CALLBACK").success(function(param) {
                    ...
                    if (canPlayM3U8 && navigator.userAgent.indexOf('PlayStation') === -1) {
                        var ep = encodeURIComponent(D(E(F(mk.a4 + "poz" + userCache.a2, dic).toString(), userCache.sid + "_" + vid + "_" + userCache.token)));
                        var oip =dataa.security.ip;
                        var source = [
                            ['超清', 'http://pl.youku.com/playlist/m3u8?vid=' + _id + '&type=hd2&ctype=12&keyframe=1&ep=' + ep + '&sid=' + userCache.sid + '&token=' + userCache.token + '&ev=1&oip=' + oip],
                            ['高清', 'http://pl.youku.com/playlist/m3u8?vid=' + _id + '&type=mp4&ctype=12&keyframe=1&ep=' + ep + '&sid=' + userCache.sid + '&token=' + userCache.token + '&ev=1&oip=' + oip],
                            ['标清', 'http://pl.youku.com/playlist/m3u8?vid=' + _id + '&type=flv&ctype=12&keyframe=1&ep=' + ep + '&sid=' + userCache.sid + '&token=' + userCache.token + '&ev=1&oip=' + oip]
                        ];
                        console.log('解析youku视频地址成功 ' + source.map(function(item) {
                            return '<a href=' + item[1] + '>' + item[0] + '</a>' }).join(' '), 2)
                        ...
                        var source = [['标清', playListData._videoSegsDic.streams['guoyu']['3gphd'][0].src]]
                        console.log('解析youku视频地址成功 ' + source.map(function(item) {
                            return '<a href=' + item[1] + '>' + item[0] + '</a>' }).join(' '), 2)
